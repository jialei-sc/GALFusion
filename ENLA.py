...
def orthogonal_matrix_chunk(cols, device = None):# cols=64
...
def gaussian_orthogonal_random_matrix(nb_rows, nb_columns, scaling = 0, device = None):
    nb_full_blocks = int(nb_rows / nb_columns) # 2
...
class ENLA(nn.Module):
    def __init__(self, dim_heads, nb_features = None, ortho_scaling = 0, generalized_attention = False, kernel_fn = nn.ReLU(), no_projection = False):
        ...
        nb_features = default(nb_features, int(dim_heads * math.log(dim_heads)))  # nb_features=128
        ...
        self.generalized_attention = generalized_attention   # False
        self.kernel_fn = kernel_fn  # nn.ReLU()
    ...
    def forward(self, q, k, v):
        ...
        if self.no_projection: # false
            q = q.softmax(dim = -1)
            k = k.softmax(dim = -2)

        elif self.generalized_attention: # false
